...
const ForgotPasswordVC = () => {
  ...
  const queryCollection = async () => {
    // Query to get the user's UID based on the email
    const q = query(
      collection(db, "virtual-assistants"),
      where("email", "==", email)
    );
    ...
    if (!querySnapshot.empty) {
      ...
      if (userSnapshot.exists()) {
        return { ...userSnapshot.data(), id: uid };
      } else {
        return null;
      }
    } else {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      let userData = await queryCollection();

      if (!userData || userData.signupType !== "Virtual Assistant") {
        toast.error("This email is not registered as a Virtual Assistant.");
        return;
      }
  ...
  return (
    <>
      <main id="content" role="main" className="w-full max-w-md p-6 mx-auto">
        <div className="bg-white border-2 border-indigo-300 shadow-lg mt-7 rounded-xl dark:bg-white dark:border-gray-700">
...
export default ForgotPasswordVC;
